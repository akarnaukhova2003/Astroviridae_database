...
```{r}
aln = read.dna("/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/Aves_full_seq_removed_1B_align.fasta", format="fasta")
ph_sp = plot_aa_nt_hists_sp(
  aln, 
  1, length(aln[1,]), 
  xlim_range = c(0, 0.2),
...
plots <- plot_hists(
  ...
  xlim_nt = c(0, 0.55),
  xlim_aa = c(0, 0.65),
  ybreak_nt = c(1250, 10000),
  ybreak_aa = c(1250, 10000),
  vlines_nt = c(0.14, 0.2),
  vlines_aa = c(0.1, 0.17),
  fill_true = "F"
)
title_plot <- ggdraw() +
  draw_label(
...
final_plot
ggsave(
  filename = "/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/ORF1B_pairwise_dist_break.png",
  plot = final_plot,
  width = 14,
  height = 6,
  dpi = 300,
...
```{r}
aln = read.dna("/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/Aves_full_seq_removed_2_align.fasta", format="fasta")
ph_sp = plot_aa_nt_hists_sp(
  aln, 
  1, length(aln[1,]), 
  xlim_range = c(0, 0.2),
...
final_plot
ggsave(
  filename = "/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/ORF2_pairwise_dist_break.png",
  plot = final_plot,
  width = 14,
  height = 6,
  dpi = 300,
...
plot_aa_nt_hists_sp = function(aln_object, pos_start, pos_end, 
  ...
  dna_slice = aln_object[, pos_start:pos_end]
  dna_slice_char = as.character.DNAbin(dna_slice)
  dna_slice_char[dna_slice_char == '-'] <- NA
  ...
  if (col_sp != 0){
    d = check_param(d, col_sp)
    
    p <- ggplot(d, aes(value.x, value.y, color = same_species)) +
...
plot_hists = function(df_dist1,
  ...
  use_fill = "same_species" %in% colnames(df_dist1)
  
  hist1_nt = ggplot(df_dist1, 
                    if (use_fill) aes(x=value.x, fill=same_species) else aes(x=value.x)) +
    geom_histogram(bins=50, alpha=0.8) +
    scale_y_break(ybreak_nt, scales = 0.6)  +
    labs(
      x = "p-дистанция по нуклеотидным последовательностям",
      y = "",
      fill = ""
    ) +
    ...
    theme_bw() +
    theme(
      axis.title.x = element_text(size=14, face="bold", margin=margin(t=8),  hjust = 0.5),
      axis.title.y = element_text(size=14, face="bold", margin=margin(r=-1)),
      axis.text.x  = element_text(size = 16),
      axis.text.y  = element_text(size = 16),
      panel.background = element_rect(fill="white"),
  ...
  hist1_aa = ggplot(df_dist1, 
                    if (use_fill) aes(x=value.y, fill=same_species) else aes(x=value.y)) +
    geom_histogram(bins=50, alpha=0.8) +
    scale_y_break(ybreak_nt, scales = 0.6) +
     labs(
      x = "p-дистанция по аминокислотным последовательностям",
      y = "",
      fill = "Сравнение"
    ) +
  ...
  if (!is.null(xlim_nt)) hist1_nt <- hist1_nt + coord_cartesian(xlim = xlim_nt)
  if (!is.null(xlim_aa)) hist1_aa <- hist1_aa + coord_cartesian(xlim = xlim_aa)
  
  return(hist1_nt + hist1_aa)
}

```

```{r}
aln = read.dna("/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/mmseq86_1B_nt/res/Aves_full_seq_removed_1A_align_86nt.fasta", format="fasta")
ph_sp = plot_aa_nt_hists_sp(
  aln, 
  1, length(aln[1,]), 
  xlim_range = c(0, 0.2),
...
final_plot
ggsave(
  filename = "/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/Aves/mmseq86_nt_ORF1A.svg",
  plot = final_plot,
  width = 14,
  height = 6,
  dpi = 300,
