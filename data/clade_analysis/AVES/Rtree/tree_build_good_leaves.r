...
plot_annotated_tree_clusters <- function(tree_file, meta_file, taxlabel=TRUE){
  ...
  if(taxlabel){
    ...
    tip_colors_map <- setNames(colors_host[host$Host], rownames(host))
    
    p <- p + geom_tiplab(aes(label = tip_labels_map[.data$label]), offset=0.02, size=1.2) 
  } else {
    p <- p + geom_treescale()
  }
  ...
  p1 <- gheatmap(p, cluster1, width=0.05, offset=0.12, colnames=TRUE, font.size=2, colnames_position="top") +
    scale_fill_manual(values=colors_f1, guide="none") + label_theme + new_scale_fill()
  
  p1 <- gheatmap(p1, cluster2, width=0.05, offset=0.24, colnames=TRUE, font.size=2, colnames_position="top") +
    scale_fill_manual(values=colors_f2, guide="none") + label_theme + new_scale_fill()
  
  p1 <- gheatmap(p1, cluster3, width=0.05, offset=0.36, colnames=TRUE, font.size=2, colnames_position="top") +
    scale_fill_manual(values=colors_f3, guide="none") + label_theme + new_scale_fill()
  
  p1 <- gheatmap(p1, cluster4, width=0.05, offset=0.48, colnames=TRUE, font.size=2, colnames_position="top") +
    scale_fill_manual(values=colors_f4, guide="none") + label_theme + new_scale_fill()
  
  p1 <- gheatmap(p1, host, width=0.05, offset=0.60, colnames=TRUE, font.size=2, colnames_position="top") +
    scale_fill_manual(values=colors_host, name="Host") + label_theme
  
  return(p1)
}

tree_file <- "/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/AVES/Aves_2_97_translated_align_new.nwk"
...
ggsave("/Users/abagavetdinova/Desktop/lab/Astroviridae_database/data/clade_analysis/AVES/2_97_tree.png",
